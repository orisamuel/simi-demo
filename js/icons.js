/* אייקונים — סט SVG אחיד בסגנון קו (stroke 2, עגול) */
window.S = window.S || {};

(function () {
  const wrap = (inner, vb) =>
    `<svg viewBox="${vb || '0 0 24 24'}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${inner}</svg>`;
  ...
  const defs = {
    ...
    'corner-up-right': '<path d="m14 15 5-5-5-5"/><path d="M4 4v7a4 4 0 0 0 4 4h11"/>',
    ...
    'arrow-back': '<path d="m9 14-4-4 4-4"/><path d="M5 10h11a4 4 0 0 1 4 4v3"/>',
    ...
    'user-swap': '<circle cx="9" cy="7" r="4"/><path d="M2 21v-2a4 4 0 0 1 4-4h6"/><path d="m16 11 3-3 3 3"/><path d="M19 8v8"/>',
  };

  S.icon = (name) => wrap(defs[name] || defs.info);

  /* החלת אייקונים על אלמנטים עם data-icon */
  S.applyIcons = (root) => {
